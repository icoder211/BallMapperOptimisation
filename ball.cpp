...
int dc[4] = {1, 0, -1, 0};
const int N = 3e5+3;
const string FILENAME = "outputs/bm";
...
int main(int argc, char** argv){   
    ...
    Nos;
    long double eps = 100; // set eps here
    // input must be CSV with headers, 
    if(argc == 2) {
        string inputfilename = argv[1];
        cerr << "Reading input from " << inputfilename << endl;
        freopen(inputfilename.c_str(), "r", stdin);
    } else if(argc == 3) {
        string inputfilename = argv[1];
        eps = stod(argv[3]);
        cerr << "Reading input from " << inputfilename << " with eps = " << eps << endl;
        freopen(inputfilename.c_str(), "r", stdin);
    } else {
        cerr << "No input file, going with boston property dataset and eps=" << eps << endl;
        string inputfilename = "R/data/boston.csv";
        freopen(inputfilename.c_str(), "r", stdin);
    }
    ...
    cerr << "Input size = " << pts.size() << ln;
    ball b = ballmap(pts, val, eps);

    // store obtained ballmapper in file
    balltofile(b);
